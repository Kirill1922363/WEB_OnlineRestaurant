...
def admin_panel():
    if request.method == "POST":
        pos_id = request.form.get("pos_id", type=int)
...
def create_menu():
    if request.method == "POST":
        ...
        if image:
            ...
            image_path = os.path.join(config.IMAGES_DIR, unique_filename)
...
def update_menu(pos_id):
    with Session() as session:
        ...
        if request.method == "POST":
            menu_pos.name = request.form.get("name")
            menu_pos.price = request.form.get("price", type=float)
            menu_pos.description = request.form.get("description")
            menu_pos.category = request.form.get("category")
            ...
            if image:
                ...
                image_path = os.path.join(config.IMAGES_DIR, unique_filename)
                image.save(image_path)
                menu_pos.image_path = image_path
